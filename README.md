...
- `pyds4.dsml` provides import-safe, framework-neutral DSML data classes plus
  ...
  invoke elements only; stray text inside a block is reported as malformed
  instead of being skipped. These helpers do not import Avalan or native DS4
  extension objects.
- `pyds4.kv_cache` provides import-safe disk KV cache key and metadata helpers
  ...
  any synchronous session with `load_payload()` and falls back to `sync()` on
  misses or corrupt entries; `store()` persists `save_payload()` bytes and
  metadata with non-fatal write errors by default. Async callers can use
  `arestore()` and `astore()` with sessions whose cache methods return
  awaitables. `evict()` applies deterministic budget eviction using hit count,
  access time, creation time, and key order.
